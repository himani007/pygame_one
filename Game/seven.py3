...
#collision and hitting boxes!---------------------
# making those bullets one at a time...shootLoops__--__- ('-_-')
import pygame
pygame.init()
...
#this loop ensures that only one bullet is shot at a time. 
	if shootLoop > 0:
		shootLoop +=1
	if shootLoop > 3:
		shootLoop = 0	
	...
	for event in pygame.event.get():
		if event.type ==pygame.QUIT:
			run = False
#this loop to show that bullet diappears when the enemy is hit... hit function is called. and bullet is removed from the bullet list.
	for bullet in bullets: 
...
#to ensure bullet is within the width of screen. 
